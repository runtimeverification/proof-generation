...
class PrettyPrinter(KoreVisitor):
    def __init__(self, output: TextIO, indentation: str = "  ", limit: int = 80, demangle: bool = True):
        """
        limit: if the encoded version exceeds this limit length, try to print arguments on a new line
        demangle: demangle kore labels
        """
    ...
    def indent(self):
        self.output.write(self.depth * self.indentation)

    def indent_and_write_line(self, content: str):
        self.indent()
        self.output.write(content)
        self.output.write("\n")

    def indent_and_write(self, content: str):
        self.indent()
        self.output.write(content)

    def demangle_label(self, label: str) -> str:
        keyword_map = {
            r"\-LT\-": "<",
            r"\-GT\-": ">",
            r"Unds": "_",
        ...
        def replacement(mangled) -> str:
            result = mangled.group(0)[1:-1]
            for key, value in keyword_map.items():
                key = key.replace("\\", "")
                result = result.replace(key, value)
            return result

        demangled = re.sub(pattern, replacement, label)

        if demangled.startswith("Lbl"):
    ...
    def postvisit_module(self, module: Module, *args):
        self.indent_and_write_line(f"module {module.name}")
        self.depth += 1
    ...
    def postvisit_variable(self, var: Variable, *args):
        self.indent_and_write(str(var))

    def postvisit_string_literal(self, literal: StringLiteral):
        self.indent_and_write(ANSI.in_magenta(str(literal)))

    def encode_symbol_instance(self, symbol: SymbolInstance) -> str:
        encoded = str(symbol)
        symbol_name = symbol.get_symbol_name()
        encoded = encoded[len(symbol_name):]
        symbol_name = self.demangle_label(symbol_name) if self.demangle else symbol_name
        encoded = ANSI.in_green(symbol_name) + encoded
        return encoded

    def postvisit_application(self, application: Application, *args):
        use_compact = len(str(application)) <= self.limit
        header = self.encode_symbol_instance(application.symbol)

        if self.demangle:
            header = self.demangle_label(header)

        if use_compact:
            self.indent_and_write(header + "(")
            old_depth = self.depth
            self.depth = 0

            for i, argument in enumerate(application.arguments):
                self.visit(argument)
                if i + 1 < len(application.arguments):
                    self.output.write(", ")
            ...
            self.indent_and_write_line(header + "(")
            self.depth += 1
    ...
    def postvisit_ml_pattern(self, ml_pattern: MLPattern, *args):
        sorts = [str(sort) for sort in ml_pattern.sorts]
        sort_str = ", ".join(sorts)

        use_compact = len(str(ml_pattern)) <= self.limit

        header = ANSI.in_blue(str(ml_pattern.construct)) + f"{{{sort_str}}}" + "("
        ...
        if use_compact:
            ...
            for i, argument in enumerate(ml_pattern.arguments):
                self.visit(argument)
                if i + 1 < len(ml_pattern.arguments):
                    self.output.write(", ")
