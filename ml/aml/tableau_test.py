...
def test_closure() -> None:
    assert closure(And(SVar("X"), EVar("Y"))) == [[SVar("X"), EVar("Y")]]
...
def test_children() -> None:
    ...
    assert SimpleNode(
        [App(SVar("X"), SVar("Y")), DApp(SVar("P"), SVar("Q"))]
    ).children() == AndNode(
        [
            OrNode(
                [
                    AndNode(
                        [SimpleNode([SVar("X")]), SimpleNode([SVar("Y"), SVar("Q")])]
                    ),
                    AndNode(
                        [SimpleNode([SVar("X"), SVar("P")]), SimpleNode([SVar("Y")])]
                    ),
                ]
            )
        ]
