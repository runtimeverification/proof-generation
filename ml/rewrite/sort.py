...
class SortProofGenerator(ProofGenerator):
    def get_inj_instance(
        ...
        sort1_is_sort = self.composer.sort_axioms[encoded_sort1].as_proof()
        sort2_is_sort = self.composer.sort_axioms[encoded_sort2].as_proof()
        sort3_is_sort = self.composer.sort_axioms[encoded_sort3].as_proof()

        copied_injection_axiom = KoreUtils.copy_ast(self.composer.module, self.composer.sort_injection_axiom.claim)
        ...
        subst_proof1, substituted1 = SingleSubstitutionProofGenerator(
            self.composer, sort_var1, sort1
        ).prove_substitution_with_result(copied_injection_axiom)

        substituted1.sort_variables = substituted1.sort_variables[1:]
        subst_proof2, substituted2 = SingleSubstitutionProofGenerator(self.composer, sort_var2, sort2
                                                                      ).prove_substitution_with_result(substituted1)

        substituted2.sort_variables = substituted2.sort_variables[1:]
        subst_proof3, substituted3 = SingleSubstitutionProofGenerator(self.composer, sort_var3, sort3
                                                                      ).prove_substitution_with_result(substituted2)

        proof1 = self.composer.get_theorem("kore-forall-sort-elim").apply(
            self.composer.sort_injection_axiom.proof,
            sort1_is_sort,
            subst_proof1,
        )

        proof2 = self.composer.get_theorem("kore-forall-sort-elim").apply(
            proof1,
            sort2_is_sort,
            subst_proof2,
        )

        proof3 = self.composer.get_theorem("kore-forall-sort-elim").apply(
            proof2,
            sort3_is_sort,
            subst_proof3,
        )
