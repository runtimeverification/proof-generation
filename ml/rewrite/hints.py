...
class RewritingHints:
    ...
    """
    def __init__(self, substitutions: List[Optional[Mapping[kore.Variable, kore.Pattern]]]):
        self.substitutions = substitutions

    def get_substitution_for_step(self, i: int) -> Optional[Mapping[kore.Variable, kore.Pattern]]:
    ...
    def load_from_object(module: kore.Module, obj: Dict[str, Any]) -> RewritingHints:
                            ...
                            schema.Optional("substitution"): [
                                {
                                    "key": schema.And(
                                        str,
                                        schema.Use(lambda key: parse_pattern(key, module)),
                                    ),
                                    "value": schema.And(
                                        str,
                                        schema.Use(lambda val: parse_pattern(val, module)),
                                    ),
                                }
                            ],
                        }
        ...
        substitutions: List[Optional[Mapping[kore.Variable, kore.Pattern]]] = [
            {item["key"]: item["value"]
             for item in action.get("substitution", [])} for action in validated.get("actions", [])
        ]
