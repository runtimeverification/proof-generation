        ...
        for label in proof:
            ...
            if steptyp in ('$a', '$p'):
                ...
                for (k, v) in mand_var:
                    ...
                    if entry[0] != k:
                        raise MMError(
                            ("stack entry ({0}, {1}) doesn't match " + "mandatory var hyp {2!s}").format(k, v, entry)
                        )
                    subst[v] = entry[1:]
                    sp += 1
...
if __name__ == '__main__':
    parser = OptionParser()
    parser.add_option('-b', '--begin-label', dest='begin_label', help='label to begin verifying')
    parser.add_option('-s', '--stop-label', dest='stop_label', help='label to stop verifying')
