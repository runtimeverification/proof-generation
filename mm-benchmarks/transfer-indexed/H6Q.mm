$c ) #ElementVariable #Symbol #Pattern BAM CXQ #Variable #SetVariable ( $.
$v ph1 ph2 kore-element-var-T ph0 kore-sort-var-S2 $.
$d kore-element-var-T kore-sort-var-S2 $.
Bg $f #Pattern ph0 $.
Bw $f #Pattern ph1 $.
CA $f #Pattern ph2 $.
BAI $a #Pattern ( BAM ph0 ph1 ph2 ) $.
CXM $a #Pattern CXQ $.
Hik $f #ElementVariable kore-sort-var-S2 $.
His $f #ElementVariable kore-element-var-T $.
Huo $a #Pattern kore-sort-var-S2 $.
HvA $a #Pattern kore-element-var-T $.
${ H6Q $p #Pattern ( BAM CXQ kore-sort-var-S2 kore-element-var-T ) $= ( CXM Huo HvA BAI )  CADBEF $. $}
