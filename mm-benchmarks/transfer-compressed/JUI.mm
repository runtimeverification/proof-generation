$c Lg #Variable ) ASI #ElementVariable CMQ #Symbol A-s #Pattern CSA wg .A rg #SetVariable ( $.
$v ph1 ph0 kore-element-var-VE4 $.
Bg $f #Pattern ph0 $.
Bw $f #Pattern ph1 $.
LQ $a #Pattern ( Lg ph0 ph1 ) $.
wQ $a #Pattern ( wg ph0 ph1 ) $.
xA $a rg ( Lg ph0 ( Lg ph1 ph0 ) ) $.
${ xw $e rg ( Lg ph0 ph1 ) $.
   yA $e rg ph0 $.
   yQ $a rg ph1 $. $}
9w $a #Pattern .A $.
ASE $a #Pattern ( ASI ph0 ph1 ) $.
A-o $a #Pattern ( A-s ph0 ) $.
CLA $f #ElementVariable kore-element-var-VE4 $.
CMM $a #Pattern CMQ $.
CR8 $a #Pattern CSA $.
CSM $a rg ( A-s CSA ) $.
JO4 $a #Pattern kore-element-var-VE4 $.
${ JUI $p rg ( Lg ( wg ( ASI kore-element-var-VE4 CMQ ) .A ) ( A-s CSA ) ) $=
  ( CR8 A-o JO4 CMM ASE 9w wQ LQ xA CSM yQ ) BCZADEFGHZMIMNJKL $. $}
