...
def main():
    print(r"""\begin{tabular}{@{\hskip 1mm}l@{\hskip 2mm}r@{\hskip 2mm}r@{\hskip 2mm}r@{\hskip 2mm}r@{\hskip 2mm}r@{\hskip 2mm}r@{\hskip 2mm}r@{\hskip 1mm}r}
&&&&& \multicolumn{4}{c}{Time (seconds)} \\
\cline{6-9}
Task & Spec. LOC & Steps & Hint Size & Proof Size & K Verifier & Gen. & Check 1 & Check 2 \\
\hline""")
    ...
    for name, config in tasks:
        ...
        kore_prove_time = get_user_time(f"stats/{config['def']}-{config['spec']}.mm.kore-prove.time")
        print(f"\\texttt{{{name}}} & {spec_loc} & {steps} & {round(hint_size / 1e6, 2)}\\,MB & {round(proof_size / 1e6)}/{round(compressed_proof_size / 1e6, 1)}\\,MB & {round(kore_prove_time, 1)} & {round(gen_time)} & {round(verify_time, 1)} & {round(verify_with_rust_metamath_time, 1)} \\\\")
